...
local function getHomophones(reading)
	...
	return require('th-hom/data')[reading] or {}
...
function export.makeList(reading)
	...
	if #homList > 1 then
		for _,term in ipairs(homList) do
			...
			if term ~= PAGENAME then
				table.insert(result, require('links').full_link({lang = lang, term = term, tr = '-'}))
			end
		end
	...
	return table.concat(result, ', ')
		.. require('utilities').format_categories(categories, lang)
