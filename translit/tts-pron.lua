...
local tNoMark = {
	["live"]       = { ["high"] = "dipping", ["mid"] = "mid", ["low"] = "high-rising"  },
	["dead-short"] = { ["high"] = "mid",     ["mid"] = "mid", ["low"] = "high-rising" },
	["dead-long"]  = { ["high"] = "low",     ["mid"] = "low", ["low"] = "falling"  },
}

local tRomMarks = {
	["high-rising"] = "́", ["mid"] = "", ["low"] = "̀",
	["dipping"] = "̌", ["falling"] = "̂", ["high-level"] = "̄",
}

local tLevels = {
	["high-rising"] = "˦˥", ["mid"] = "˧", ["low"] = "˨˩",
	["rising"] = "˧˨˧", ["falling"] = "˥˩", ["high-level"] = "˥",
}
...
local note_set = {
	...
	["unorthographical"] = annotate(
		"Unorthographical", 
		"This phonetic respelling violates Isan alphabet rules to indicate an irregular pronunciation."
	),
}

function getRhymeFromIPA(ipa)
...
function export.show(frame)
	...
	if match(p_concatenated, "็" .. Thai_range .. "*[่้๊๋]")
		or match(p_concatenated, "[่้๊๋]" .. Thai_range .. "*็") 
	...
	function formatIsan(text, pron, qualifiers)
		...
		return ((qualifiers and table.concat(qualifiers) ~= "")
			...
			or '') .. '<span lang="tts" class="Isan ' .. readclass .. '">' .. text .. '</span>'
	end
		...
		local function f(text, system)
			...
			elseif system == "ipa" then
				...
				return '<span class="IPA">/' .. text .. '/</span>' ..
					(rhyme and ("<sup>([[Rhymes:Isan/" .. rhyme .. "|R]])</sup>") or "")
			...
			table.insert(result[system], tostring( mw.html.create( "td" )
				...
				:wikitext( 
					...
					and      tostring( mw.html.create( "div" )
                        ...
                        :wikitext( formatIsan(spelling, false, qualifiers) .. getCharSeq(spelling) ) )
	...
	if find(pagename, "ทร") and find(table.concat(result["charPhon"]), "ซ") then
		table.insert(categories, "[[Category:Isan terms spelled with ทร read as ซ]]")
	end
	...
	((namespace ~= "" and not args.pagename)
		...
		or (pagename == table.concat(p)
			and row(
				...
				formatIsan(pagename,true) .. getCharSeq(pagename),
				{ ["ortho"] = true })
	...
	row("''[[Wiktionary:Isan Romanization|Romanization]]''", nil, { ["rom"] = true }) ..
	
	row(
		"''[[Wiktionary:Isan Romanization|Paiboon]]''",
		table.concat(result["paiboon"]),
		{ ["pr"] = true }) ..
	
	row(
		"''[[Wiktionary:Isan Romanization|Royal Institute]]''",
		table.concat(result["royin"]),
		{ ["pr"] = true }) ..
	
	row(
		"(''[[w:Isan language|standard]]'') " ..
