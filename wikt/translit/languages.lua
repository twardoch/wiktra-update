...
function Language:getFamily()
	...
	if family then
		self._familyObject = require("families").getByCode(family)
	end
	...
	if not self._ancestorObjects then
		...
		if self._rawData.ancestors then
			for _, ancestor in ipairs(self._rawData.ancestors) do
				table.insert(self._ancestorObjects, export.getByCode(ancestor) or require("etymology languages").getByCode(ancestor))
			end
		else
...
function Language:transliterate(text, sc, module_override)
	...
	if module_override then
		require("debug").track("module_override")
	end
...
function export.getByCode(code, paramForError, allowEtymLang, allowFamily)
	...
	if not retval and allowEtymLang then
		retval = require("etymology languages").getByCode(code)
	end
	if not retval and allowFamily then
		retval = require("families").getByCode(code)
	end
...
function export.getByCanonicalName(name, errorIfInvalid, allowEtymLang, allowFamily)
	...
	if not retval and allowEtymLang then
		retval = require("etymology languages").getByCanonicalName(name)
	end
	if not retval and allowFamily then
...
function export.getNonEtymological(lang)
	while lang:getType() == "etymology language" do
		...
		lang = export.getByCode(parentCode)
			or require("etymology languages").getByCode(parentCode)
			or require("families").getByCode(parentCode)
	end
